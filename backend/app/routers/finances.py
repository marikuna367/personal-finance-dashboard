...
from ..bank_client import (
    get_transactions,
    get_account_info,
    create_transaction as provider_create_transaction,
)
        ...
        for t in provider_txns:
            ...
            exists = session.exec(
                select(models.Transaction).where(
                    models.Transaction.provider_txn_id == provider_id
                )
            ).first()
            if exists:
                continue
            ...
            date_str = (
                t.get("date") or t.get("timestamp") or t.get("created_at") or None
            )
            if date_str:
                try:
...
async def create_txn_body(txn: schemas.TransactionIn):
    with Session(db.engine) as session:
        ...
        try:
            await provider_create_transaction(
                new_txn.account_id,
                new_txn.amount,
                new_txn.category or "",
                new_txn.description or "",
            )
        except Exception:
            pass
        return new_txn
